...
/**
 * Get the builder rank from the guild member roles, if they have a builder role, else return -1 (as for junior builder)
...
/**
 * Get the role id associated with the rank 
 * @param rank Rank from [0 to 4]
 * @param guild The guild
 */
async function getBuilderRoleFromRank(rank: number, guild: Guild, guildData: GuildInterface) {
    return await guild.roles.fetch( guildData[`rank${rank + 1}`].id)
}

export { getBuilderRankFromRoles, getBuilderRoleFromRank }